...
class MetaWareNNFunction final : public CompiledFunction {
 public:
  ...
  // Reads the glow constant tensor and creates MWNN initializer tensor
  template<class T1, class T2>
  void ReadTensor(glow::Constant *c, std::string tensor_name,
                  ElemKind elem_kind);
  // Creates scale and zero point as MWNN initializer tensor from glow NodeValue
  void CreateMWNNQuantParams(NodeValue c, std::string tensor_name);
  // Creates MWNN Node from the passed params
  ...
  // Creates Quantize and Dequantize node
  void CreateQDQNodes(std::string ip_name, std::string op_name,
                      std::string node_name);
  // Maps the Glow Element type to MWNN Element type
